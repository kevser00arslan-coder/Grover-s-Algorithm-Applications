...
# eine einzelne Phasen-Umkehrung fuer einen Zielzustand
...
# Orakel: dreht die Phase ALLER Zielzustaende um
def oracle(qc):
    for ziel in ziele:
        markiere(qc, ziel)

# Diffusion: spiegelt die Amplituden am Mittelwert
def diffusion(qc):
    qc.h(range(n))
    qc.x(range(n))
    qc.h(n - 1)
...
# optimale Anzahl haengt von der Zahl der Loesungen ab
M = len(ziele)
...
print("Anzahl verschiedener Zustaende:", len(counts))
for z in ziele:
